...
class ManipulatorEnv:
    ...
    def ax_render(self, ax) -> None:
        """
        Displays current configuration.
        :param plt_show: whether to call plt.show() or not
        """
        ax = self._plot_segment(self._state.joints[[0, 1], :], np.array([1, 0, 0]),
                           is_start_link=True)
        ax = self._plot_segment(self._state.joints[[1, 2], :], np.array([0, 1, 0]))
        ax = self._plot_segment(self._state.joints[[2, 3], :], np.array([0, 0, 1]))
        ax = self._plot_segment(self._state.joints[[3, 4], :], np.array([1, 0, 1]),
                           is_end_link=True)
        for obs in self._obstacles:
            ax = plt.gca().add_patch(
                plt.Circle((obs[0], obs[1]), obs[2], fill=True))
        plt.axis('equal')
